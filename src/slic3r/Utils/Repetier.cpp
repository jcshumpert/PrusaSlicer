    ...
    http.on_error([&](std::string body, std::string error, unsigned status) {
        ...
        .on_complete([&, this](std::string body, unsigned) {
            ...
            try {
                ...
                if (! res) {
                    msg = wxString::Format(_(L("Mismatched type of print host: %s")), text ? *text : "Repetier");
                }
            }
...
wxString Repetier::get_test_failed_msg (wxString &msg) const
{
    return wxString::Format("%s: %s\n\n%s",
        _(L("Could not connect to Repetier")), msg, _(L("Note: Repetier version at least 1.1.0 is required.")));
}
