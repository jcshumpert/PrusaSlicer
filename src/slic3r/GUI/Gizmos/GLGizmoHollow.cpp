...
void GLGizmoHollow::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    m_desc["offset"]  = wxString::Format("%s%s", _(opts[0].second->label), ":");
    m_desc["quality"] = wxString::Format("%s%s", _(opts[1].second->label), ":");
    m_desc["closing_distance"] = wxString::Format("%s%s", _(opts[2].second->label), ":");
